...
class Textures(object):
    ...
    def __init__(self, texturepath=None, cachepath=None, bgcolor=(26, 26, 26, 0), northdirection=0):
        ...
        self.rasterizer = Rasterizer(self, self.texture_dimensions)
    ...
    def index_blocks(self):
        # when we're at it, replace some existing blocks that are rendered incorrectly
        replace_blocks = set([
            "minecraft:lectern",
                    ...
                    if "multipart" in blockstates:
                        ...
                        for part in blockstates["multipart"]:
                            ...
                            for k,v in when.items():
                                ...
                                # make extra sure we have all combinations
                                if v == "false":
                                    kv[k].add("true")
                                if v == "true":
                                    kv[k].add("false")
                        ...
                        if full_name == "minecraft:bamboo":
                            kv["leaves"].add("none")
                        if full_name[-5:] == "_wall":
                            for k in ["up","north","east","south","west"]:
                                kv[k].add("none")
                        
                        for variant in list(itertools.product(*kv.values())):
                            props = dict(zip(kv.keys(), variant))
                            block_data = ','.join([
                                '%s=%s' % kv
    ...
    def generate(self):
        ...
        for blockid, img in self.newblocks.items():
            self.blockmap[blockid * max_data + 0] = self.generate_texture_tuple(img)
            #if img:
            #    img.save("../test-%d.png" % blockid)
    ...
    def load_json(self, filename):
        if filename in self.json_cache:
            return self.json_cache[filename]
        
        fileobj = self.find_file(filename)
        data = json.load(fileobj)
        self.json_cache[filename] = data
        return data
...
# signposts
@material(blockid=[63,11401,11402,11403,11404,11405,11406,25000,25002], data=list(range(16)), transparent=True)
def signpost(self, blockid, data):
    ...
    sign_texture = {
        ...
        25000: ("warped_planks.png", "warped_stem.png"),
        25002: ("crimson_planks.png", "crimson_stem.png"),
    }
...
# wall signs
@material(blockid=[68,11407,11408,11409,11410,11411,11412,25001,25003], data=[2, 3, 4, 5], transparent=True)
def wall_sign(self, blockid, data): # wall sign
    ...
    sign_texture = {
        ...
        25001: "warped_planks.png",
        25003: "crimson_planks.png",
    }
